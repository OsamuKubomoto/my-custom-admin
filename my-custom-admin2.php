...
<form id="my-submenu-form" method="post" action="">
	...
	<p>
		<?php echo esc_html( __('E-mail Address', 'my-custom-admin') ); ?>
		<input type="text" name="my-data" value="<?php echo esc_attr( get_option( 'my-data' ) ); ?>">
	</p>
	<p>
		<input type="submit" value="<?php echo esc_attr( __('Save', 'my-custom-admin' ) ); ?>" class="button button-primary button-large">
	</p>
		...
		if ( check_admin_referer( 'my-nonce-key', 'my-submenu' ) ) {
			...
			if ( isset($_POST['my-data']) && $_POST['my-data'] ) {
				if ( is_email( trim( $_POST['my-data'] ) ) ) {
					...
					$e->add( 'error', __('Please enter a valid email address.', 'my-custom-admin' ) );
